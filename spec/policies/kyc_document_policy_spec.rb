# frozen_string_literal: true

require "rails_helper"

RSpec.describe KycDocumentPolicy, type: :policy do
  let(:psp_admin)      { build(:user, :psp_admin) }
  let(:psp_support)    { build(:user, :psp_support) }
  let(:merchant_admin) { build(:user, :merchant_admin) }
  let(:document)       { build(:kyc_document) }

  it("psp_admin can create")      { expect(described_class.new(psp_admin,      document).create?).to be true }
  it("psp_support cannot create") { expect(described_class.new(psp_support,    document).create?).to be false }
  it("psp_support can show")      { expect(described_class.new(psp_support,    document).show?).to be true }
  it("merchant_admin cannot show") { expect(described_class.new(merchant_admin, document).show?).to be false }

  it("psp_admin can confirm_dates")        { expect(described_class.new(psp_admin,      document).confirm_dates?).to be true }
  it("psp_support cannot confirm_dates")   { expect(described_class.new(psp_support,    document).confirm_dates?).to be false }
  it("merchant_admin cannot confirm_dates") { expect(described_class.new(merchant_admin, document).confirm_dates?).to be false }

  it("psp_admin can resolve_dob_mismatch")        { expect(described_class.new(psp_admin,      document).resolve_dob_mismatch?).to be true }
  it("psp_support cannot resolve_dob_mismatch")   { expect(described_class.new(psp_support,    document).resolve_dob_mismatch?).to be false }
  it("merchant_admin cannot resolve_dob_mismatch") { expect(described_class.new(merchant_admin, document).resolve_dob_mismatch?).to be false }

  it("psp_admin can update_comment_status")        { expect(described_class.new(psp_admin,      document).update_comment_status?).to be true }
  it("psp_support can update_comment_status")      { expect(described_class.new(psp_support,    document).update_comment_status?).to be true }
  it("merchant_admin cannot update_comment_status") { expect(described_class.new(merchant_admin, document).update_comment_status?).to be false }
end
